...
def main():
    ...
    if cfg.model.backbone in [
        ...
        raise ValueError(f"Not supported model: cfg.model.backbone={cfg.model.backbone}")

    # Reduce batch size by half to mitigate memory issues
    reduced_batch_size = max(1, cfg[mode].batch_size // 2)
    ...
    data_loader = DataLoader(
        dataset,
        batch_size=reduced_batch_size,  # Use reduced batch size
        shuffle=False,
    ...
    if cfg.task != 'analysis':
        for task_name, score_task in scores.items():
            print(f"{task_name} --> precision: {score_task['precision']:.4f}, recall: {score_task['recall']:.4f}, f1: {score_task['f1']:.4f}")
    else:
        print('eval: | ', end='')
...
def load_model_weight(net, pretrained_model_file):
    ...
    missing_keys = set(valid_keys) - set(new_state_dict.keys())
    if missing_keys:
        print(f"These keys are missing and will be randomly initialized: {missing_keys}")
